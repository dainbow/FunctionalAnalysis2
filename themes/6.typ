...
#proposition[
  Если $A in cal(K)(E_1, E_2)$, E_2 - банахово и $x_n weak x_0 in E_1$, то $A x_n -> A x_0$.
]
...
#proof[
  ...
  Далее, предположим противное. Пусть не сходится. Тогда $exists epsilon exists A x_n_k, norm(A x_n_k - A x_0) > epsilon$. 
  Заметим, что $seq(x)$ предкомпакт, а значит из $x_n_k$ можно выделить сходящуюся подпоследовательность $A x_n_k_l -> y$. 
  Так как отделена от $A x_0$, то $y != A x_0$

  Из сходимости следует слабая, то есть $A x_n_k_l weak y$, но в то же время $A x_n_k_l weak A x_0 != y$. Противоречие.
]
...
#theorem(
  ...
  Пусть $H$ -- сепарабельное гильбертово пространство над полем $CC$, $A$ -- компактный
  самосопряжённый оператор. Тогда в $H$ найдётся ортонормированный базис,
  состоящий из собственных векторов оператора $A$.
]

#proof[
  Построим нужный базис явным образом. Для этого упорядочим все собственные
  значения оператора $A$ по модулю, причём включим в этот ряд копии этих значений
  столько раз, сколько соответствует размерности их собственного подпространства
  (в силу теоремы о конечности размерности собственных подпространств, это
  возможно). Получим ряд:
  #eq[
    $abs(lambda_1) >= abs(lambda_2) >= abs(lambda_3) >= ...$
  ]
  Пусть $v_n$ -- нормированный собственный вектор, соответствующий $lambda_n$ (для
  равных СЗ берём ортонормированные вектора базиса подпространства).

  Образуем ортонормированную систему $seq(e)$, полученную перенумерованием
  вектором $v_n$ и добавлением собственных векторов, соответствующих $lambda = 0$ (конечно,
  если оно является СЗ).

  Так как мы находимся в сепарабельном пространстве, то для того, чтобы эта
  система была базисом, достаточно доказать её полноту. Обозначим $M = [angle.l seq(e) angle.r]$.
  Коль скоро это подпространство, можно применить теорему о проекции:
  #eq[
    $M plus.circle M^bot = H$
  ]
  Стало быть, $M = H <=> M^bot = {0}$. Покажем, что $M^bot$ инвариантно
  относительно $A$.

  В силу самосопряжённости $A$, достаточно это доказать для просто $M$ (лемма об
  инвариантности).

  Введём дополнительное обозначение $L := angle.l seq(e) angle.r$. Тогда $A L subset.eq L$ тривиальным
  образом. При этом оператор $A$ компактен, а значит непрерывен, то есть
  #eq[
  ...
  - $tilde(A) = 0$. Этот факт можно записать следующим образом:
    ...
    Стало быть, $M^bot subset.eq "Ker" tilde(A)$. Но так как мы рассмотрели сужение
    на $M^bot$, то по определению $M$ мы оставили $"Ker" A without {0}$ за бортом,
    то есть $"Ker" tilde(A) = {0} = M^bot$.
  - $tilde(A) != 0$. Предположим противное: $M^bot != {0}$.

    По доказанной выше лемме, у $tilde(A)$ существует ненулевое СЗ $lambda$.
    Обозначим за $e$ -- соответствующий нормированный собственный вектор, то есть $tilde(A)e = lambda e$,
    но ведь тогда и $A e = lambda e$. Получили противоречие с определением $M$.
]
